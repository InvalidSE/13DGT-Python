...
def get_options():
    ...
    amount = verify_input(input("\nHow many questions do you want? "), 1, max_questions)

    # get the amount of players
    players = verify_input(input("\nHow many players are there? "), 1, max_users)
...
def quiz(questions, player_number, users):
    ...
    for i, question in enumerate(questions):
        ...
        if correct_answer_position+1 == answer:
            ...
            print(colours.GREEN + "\nCorrect!" + colours.ENDC)
        else:
            print(colours.RED + "\nIncorrect!" + f"\nThe correct answer was {question.correct_answer}." + colours.ENDC)
...
def final_results(users, questions):
    ...
    for i, user in enumerate(users):
        ...
        for j, question in enumerate(user.answers):
            if question["correct"]:
                answer_string += colours.GREEN + "•" + colours.ENDC
            else:
                answer_string += colours.RED + "•" + colours.ENDC
            if j == len(user.answers)-1:
                answer_string += "]"

        leaderboard_string = (f"{i+1}. {user.name}: {user.score}/{len(questions)} " + colours.ENDC)
...
def view_previous_quiz():
    ...
    for i, user in users.items():
        for j, answer in enumerate(user["answers"]):
            ...
            print(f"{colours.BOLD}Question {j+1}: {answer['question']}{colours.ENDC}")
            if(answer["correct"]):
                print(colours.GREEN + "User answer: " + str({answer['user_answer']})[2:-2] + colours.ENDC)
            else:
                print(colours.RED + "User answer: " + str({answer['user_answer']})[2:-2] + colours.ENDC)
                print(f"{colours.CYAN}Correct answer: {answer['correct_answer']}{colours.ENDC}")
